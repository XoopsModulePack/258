...
class TCPDF_PARSER {
    ...
    public function __construct($data, $cfg=array()) {
        if (empty($data)) $this->Error('Empty PDF data.');
        // find the pdf header starting position
        if (($trimpos = strpos($data, '%PDF-')) === FALSE) $this->Error('Invalid PDF data: missing %PDF header.');
        ...
        foreach ($this->xref['xref'] as $obj => $offset) {
            if (!isset($this->objects[$obj]) AND ($offset > 0)) $this->objects[$obj] = $this->getIndirectObject($obj, $offset, true);
        }
        // release some memory
        unset($this->pdfdata);
        $this->pdfdata = '';
    ...
    protected function setConfig($cfg) {
        if (isset($cfg['die_for_errors'])) $this->cfg['die_for_errors'] = !!$cfg['die_for_errors'];
        if (isset($cfg['ignore_filter_decoding_errors'])) $this->cfg['ignore_filter_decoding_errors'] = !!$cfg['ignore_filter_decoding_errors'];
        if (isset($cfg['ignore_missing_filter_decoders'])) $this->cfg['ignore_missing_filter_decoders'] = !!$cfg['ignore_missing_filter_decoders'];
    }
    ...
    protected function getXrefData($offset=0, $xref=array()) {
        ...
        if ($offset == 0) {
            // find last startxref
            if (preg_match_all('/[\r\n]startxref[\s]*[\r\n]+([0-9]+)[\s]*[\r\n]+%%EOF/i', $this->pdfdata, $matches, PREG_SET_ORDER, $offset) == 0) $this->Error('Unable to find startxref');
        ...
        if (empty($xref)) $this->Error('Unable to find xref');
        return $xref;
    }
    protected function decodeXref($startxref, $xref=array()) {
        ...
        while (preg_match('/([0-9]+)[\x20]([0-9]+)[\x20]?([nf]?)(\r\n|[\x20]?[\r\n])/', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
            if ($matches[0][1] != $offset) break;
            $offset += strlen($matches[0][0]);
            if ($matches[3][0] == 'n') {
                // create unique object index: [object number]_[generation number]
                $index = $obj_num.'_'.intval($matches[2][0]);
                // check if object already exist
                if (!isset($xref['xref'][$index])) $xref['xref'][$index] = intval($matches[1][0]);
                ++$obj_num;
        ...
        if (preg_match('/trailer[\s]*<<(.*)>>/isU', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
            ...
            if (!isset($xref['trailer']) OR empty($xref['trailer'])) {
                ...
                $xref['trailer'] = array();
                // parse trailer_data
                if (preg_match('/Size[\s]+([0-9]+)/i', $trailer_data, $matches) > 0) $xref['trailer']['size'] = intval($matches[1]);
                if (preg_match('/Root[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) $xref['trailer']['root'] = intval($matches[1]).'_'.intval($matches[2]);
                if (preg_match('/Encrypt[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) $xref['trailer']['encrypt'] = intval($matches[1]).'_'.intval($matches[2]);
                if (preg_match('/Info[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) $xref['trailer']['info'] = intval($matches[1]).'_'.intval($matches[2]);
            ...
            if (preg_match('/Prev[\s]+([0-9]+)/i', $trailer_data, $matches) > 0) $xref = $this->getXrefData(intval($matches[1]), $xref);
        } else {
    ...
    protected function decodeXrefStream($startxref, $xref=array()) {
        ...
        unset($predictor,$index_entries);
        if (isset($prevxref)) $xref = $this->getXrefData($prevxref, $xref);
        return $xref;
    }
    ...
    protected function decodeStream($sdic, $stream) {
        ...
        if ($slength <= 0) return array('', array());
        $filters = array();
        foreach ($sdic as $k => $v) {
            if ($v[0] == '/') {
