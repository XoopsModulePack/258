...
class TCPDFBarcode {
    ...
    protected function encode_code39_ext($code) {
        ...
        $clen = strlen($code);
        for ($i = 0 ; $i < $clen; ++$i) {
            if (ord($code{$i}) > 127) return false;
    ...
    protected function barcode_code93($code) {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            $char = ord($code{$i});
            if(!isset($chr[$char])) return false;
            for ($j = 0; $j < 6; ++$j) {
                if (($j % 2) == 0) {
                    $t = true; // bar
                } else {
    ...
    protected function barcode_msi($code, $checksum=false) {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            $digit = $code{$i};
            if (!isset($chr[$digit])) return false;
            $seq .= $chr[$digit];
        }
    ...
    protected function barcode_s25($code, $checksum=false) {
        ...
        if ($checksum) $code .= $this->checksum_s25($code);
    ...
    protected function barcode_i25($code, $checksum=false) {
        ...
        for ($i = 0; $i < $clen; $i = ($i + 2)) {
            $char_bar = $code{$i};
            $char_space = $code{$i+1};
            if((!isset($chr[$char_bar])) OR (!isset($chr[$char_space]))) return false;
    ...
    protected function barcode_postnet($code, $planet=false) {
        ...
        $len = strlen($code);
        // start bar
        $bararray['bcode'][] = array('t' => 1, 'w' => 1, 'h' => 2, 'p' => 0);
        $bararray['bcode'][] = array('t' => 0, 'w' => 1, 'h' => 2, 'p' => 0);
        $bararray['maxw'] += 2;
        for ($i = 0; $i < $len; ++$i) {
            for ($j = 0; $j < 5; ++$j) {
                $h = $barlen[$code{$i}][$j];
                $p = floor(1 / $h);
                $bararray['bcode'][] = array('t' => 1, 'w' => 1, 'h' => $h, 'p' => $p);
                $bararray['bcode'][] = array('t' => 0, 'w' => 1, 'h' => 2, 'p' => 0);
                $bararray['maxw'] += 2;
            }
        }
    ...
    protected function barcode_rms4cc($code, $kix=false) {
        ...
        if ($notkix) {
            // stop bar
            $bararray['bcode'][] = array('t' => 1, 'w' => 1, 'h' => 3, 'p' => 0);
            $bararray['maxw'] += 1;
        }
        return $bararray;
    }
