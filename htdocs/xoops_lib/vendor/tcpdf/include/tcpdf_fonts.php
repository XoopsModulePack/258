...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (!isset($font_path_parts['filename'])) $font_path_parts['filename'] = substr($font_path_parts['basename'], 0, -(strlen($font_path_parts['extension']) + 1));
		...
		if (empty($font_name)) $font_name = 'tcpdffont';
		// set output path
		if (empty($outpath)) $outpath = self::_getfontpath();
		// check if this font already exist
		if (@file_exists($outpath.$font_name.'.php')) return $font_name;
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['italicAngle'] != 0) $fmetric['Flags'] |= 64;
			...
			if ($matches[1] == 'true') $fmetric['Flags'] |= 1;
			// get internal map
			$imap = array();
			...
			if (preg_match('#/ForceBold[\s]*([^\s]*)#', $eplain, $matches) > 0) {
				if ($matches[1] == 'true') $fmetric['Flags'] |= 0x40000;
			}
			...
			for ($cid = 0; $cid <= 255; ++$cid) {
				if (isset($cwidths[$cid])) {
					if ($cwidths[$cid] > $fmetric['MaxWidth']) $fmetric['MaxWidth'] = $cwidths[$cid];
			...
			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x10000) return false;
			$offset += 4;
			...
			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x5F0F3CF5) return false;
			$offset += 4;
			$offset += 2; // skip flags
			// get FUnits
			...
			// PDF font flags
			$fmetric['Flags'] = $flags;
			if (($macStyle & 2) == 2) $fmetric['Flags'] |= 64;
			...
			//$offset += 2;
			if ($isFixedPitch) $fmetric['Flags'] |= 1;
			// ---------- get hhea data ----------
			...
			if ($numberOfHMetrics < $numGlyphs) $cw = array_pad($cw, $numGlyphs, $cw[($numberOfHMetrics - 1)]);
			$fmetric['MissingWidth'] = $cw[0];
			$fmetric['cw'] = '';
			$fmetric['cbbox'] = '';
			for ($cid = 0; $cid <= 65535; ++$cid) {
				if (isset($ctg[$cid])) {
					if (isset($cw[$ctg[$cid]])) $fmetric['cw'] .= ','.$cid.'=>'.$cw[$ctg[$cid]];
		...
		if (($fmetric['type'] == 'TrueTypeUnicode') AND (count($ctg) == 256)) $fmetric['type'] = 'TrueType';
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		ksort($subsetchars);
		$offset = 0; // offset position of the font data
		if (TCPDF_STATIC::_getULONG($font, $offset) != 0x10000) return $font;
		$offset += 4;
		// get number of tables
		$numTables = TCPDF_STATIC::_getUSHORT($font, $offset);
		$offset += 2;
		...
		// check magicNumber
		$offset = $table['head']['offset'] + 12;
		if (TCPDF_STATIC::_getULONG($font, $offset) != 0x5F0F3CF5) return $font;
		//$offset += 4;
		// get offset mode (indexToLocFormat : 0 = short, 1 = long)
		$offset = $table['head']['offset'] + 50;
		$short_offset = (TCPDF_STATIC::_getSHORT($font, $offset) == 0);
			...
			switch ($format) {
				...
				case 2: { // Format 2: High-byte mapping through table
					...
					for ($i = 0; $i < 256; ++$i) {
						...
						$subHeaderKeys[$i] = (TCPDF_STATIC::_getUSHORT($font, $offset) / 8);
						$offset += 2;
						if ($numSubHeaders < $subHeaderKeys[$i]) $numSubHeaders = $subHeaderKeys[$i];
					}
					// the number of subHeaders is equal to the max of subHeaderKeys + 1
					++$numSubHeaders;
					// read subHeader structures
				...
				case 8: { // Format 8: Mixed 16-bit and 32-bit coverage
					...
					for ($i = 0; $i < $nGroups; ++$i) {
						...
						for ($k = $startCharCode; $k <= $endCharCode; ++$k) {
							...
							if (isset($subsetchars[$c])) $subsetglyphs[$startGlyphID] = true;
							++$startGlyphID;
						}
					}
					break;
				...
				case 12: { // Format 12: Segmented coverage
					...
					for ($k = 0; $k < $nGroups; ++$k) {
						...
						$offset += 4;
						for ($c = $startCharCode; $c <= $endCharCode; ++$c) {
							if (isset($subsetchars[$c])) $subsetglyphs[$startGlyphCode] = true;
							++$startGlyphCode;
						}
					}
					break;
		...
		foreach ($table as $tag => $val) {
			if (in_array($tag, $table_names)) {
				...
				if ($tag == 'head') $table[$tag]['data'] = substr($table[$tag]['data'], 0, 8)."\x0\x0\x0\x0".substr($table[$tag]['data'], 12);
	...
	public static function _putfontwidths($font, $cidoffset=0) {
		...
		foreach ($font['cw'] as $cid => $width) {
			$cid -= $cidoffset;
			if ($font['subset'] AND (!isset($font['subsetchars'][$cid]))) continue;
			if ($width != $font['dw']) {
				if ($cid == ($prevcid + 1)) {
					// consecutive CID
					if ($width == $prevwidth) {
	...
	public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
		if ($isunicode) return array_map(array('TCPDF_FONTS', 'unichrUnicode'), $ta);
	...
	public static function UTF8ArrSubString($strarr, $start='', $end='', $unicode=true) {
		...
		if (strlen($end) == 0) $end = count($strarr);
		$string = '';
		for ($i = $start; $i < $end; ++$i) {
	...
	public static function UniArrSubString($uniarr, $start='', $end='') {
		if (strlen($start) == 0) $start = 0;
		if (strlen($end) == 0) $end = count($uniarr);
		$string = '';
		for ($i=$start; $i < $end; ++$i) {
			$string .= $uniarr[$i];
		}
	...
	public static function UTF8ToUTF16BE($str, $setbom=false, $isunicode=true, &$currentfont) {
		if (!$isunicode)return $str; // string is not in unicode
		$unicode = self::UTF8StringToArray($str, $isunicode, $currentfont); // array containing UTF-8 unicode values
	...
	public static function utf8Bidi($ta, $str='', $forcertl=false, $isunicode=true, &$currentfont) {
		...
		// max level
		$maxlevel = 0;
		if (TCPDF_STATIC::empty_string($str)) $str = self::UTF8ArrSubString($ta, '', '', $isunicode);
		...
		if (!($forcertl OR $arabic OR preg_match(TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $str))) return $ta;
		...
		for ($i=0; $i < $numchars; ++$i) {
			...
			if ($odd) {
				if (($chardata[$i]['type'] == 'L') OR ($chardata[$i]['type'] == 'AN') OR ($chardata[$i]['type'] == 'EN')) $chardata[$i]['level'] += 1;
			} else {
		...
		if ($arabic) {
				...
				if (TCPDF_FONT_DATA::$uni_type[$thischar['char']] == 'AL') {
					...
					if (($prevchar !== false) AND ($nextchar !== false) AND
						...
						if (in_array($prevchar['char'], $endedletter)) {
							...
							if (isset($arabicarr[$thischar['char']][3])) $chardata2[$i]['char'] = $arabicarr[$thischar['char']][3];
						}
					} elseif (($nextchar !== false) AND
						((TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'AL') OR (TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM')) AND
						($nextchar['type'] == $thischar['type']) AND
						($nextchar['char'] != 1567)) {
						if (isset($arabicarr[$chardata[$i]['char']][2])) $chardata2[$i]['char'] = $arabicarr[$thischar['char']][2];
						...
						if (($i > 1) AND ($thischar['char'] == 1607) AND
							...
							if (($prevchar !== false) AND in_array($prevchar['char'], $endedletter)) {
								if (isset($arabicarr[$thischar['char']][0])) $chardata2[$i]['char'] = $arabicarr[$thischar['char']][0];
							} else {
								if (isset($arabicarr[$thischar['char']][1])) $chardata2[$i]['char'] = $arabicarr[$thischar['char']][1];
							}
						}
					...
					// if laa letter
					if ($laaletter) $chardata2[($charAL[($x-1)]['i'])]['char'] = false;
			...
			foreach ($chardata2 as $key => $value) {
				if ($value['char'] === false) unset($chardata2[$key]);
			}
		...
		for ($j=$maxlevel; $j > 0; $j--) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				if ($chardata[$i]['level'] >= $j) {
					$onlevel = true;
					if (isset(TCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']])) $chardata[$i]['char'] = TCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']];
					$revarr[] = $chardata[$i];
				} else {
					if ($onlevel) {
