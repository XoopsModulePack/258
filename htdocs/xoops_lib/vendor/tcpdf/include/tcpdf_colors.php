...
class TCPDF_COLORS {
	...
	public static function getSpotColor($name, &$spotc) {
		if (isset($spotc[$name])) return $spotc[$name];
	...
	public static function convertHTMLColorToDec($hcolor, &$spotc, $defcol=array('R'=>128,'G'=>128,'B'=>128)) {
		...
		if (strpos($color, '[') !== false) {
			...
			if ($color == 'transparent') return array();
		}
		if (strlen($color) == 0) return $defcol;
		// RGB ARRAY
		...
		if ($color[0] != '#') {
			...
			if (isset(self::$webcolor[$color])) {
				...
				if ($returncolor === false) $returncolor = $defcol;
				return $returncolor;
			}
		} else {
	...
	public static function _JScolor($color) {
		if (substr($color, 0, 1) == '#') return sprintf("['RGB',%F,%F,%F]", (hexdec(substr($color, 1, 2)) / 255), (hexdec(substr($color, 3, 2)) / 255), (hexdec(substr($color, 5, 2)) / 255));
		if (!in_array($color, self::$jscolor)) $color = self::$jscolor[0];
		return 'color.'.$color;
	}
}