...
class QRcode {
	...
	public function __construct($code, $eclevel = 'L') {
		...
		if ($this->level === false) $this->level = QR_ECLEVEL_L;
		if (($this->hint != QR_MODE_8B) AND ($this->hint != QR_MODE_KJ)) return false;
		if (($this->version < 0) OR ($this->version > QRSPEC_VERSION_MAX)) return false;
		$this->items = array();
	...
	protected function encodeString($string) {
		$this->dataStr = $string;
		if (!$this->casesensitive) $this->toUpper();
	...
	protected function encodeMask($mask) {
		...
		if (is_null($this->datacode)) return NULL;
	...
	protected function generateMaskNo($maskNo, $width, $frame) {
	 ...
	 protected function identifyMode($pos) {
		if ($pos >= strlen($this->dataStr)) return QR_MODE_NL;
		$c = $this->dataStr[$pos];
		if ($this->isdigitat($this->dataStr, $pos)) {
			...
			if ($pos+1 < strlen($this->dataStr)) {
				...
				$word = (ord($c) << 8) | ord($d);
				if (($word >= 0x8140 && $word <= 0x9ffc) OR ($word >= 0xe040 && $word <= 0xebbf)) return QR_MODE_KJ;
			}
		}
		return QR_MODE_8B;
	}
	 ...
	 protected function newInputItem($mode, $size, $data, $bstream=null) {
		...
		if (!$this->check($mode, $size, $setData)) return NULL;
		$inputitem = array();
		$inputitem['mode'] = $mode;
		$inputitem['size'] = $size;
	...
	protected function appendNewInputItem($items, $mode, $size, $data) {
	 ...
	 protected function insertStructuredAppendHeader($items, $size, $index, $parity) {
		if ($size > MAX_STRUCTURED_SYMBOLS) return -1;
		if (($index <= 0) OR ($index > MAX_STRUCTURED_SYMBOLS)) return -1;
	 ...
	 protected function checkModeNum($size, $data) {
		for ($i=0; $i<$size; ++$i) {
			if ((ord($data[$i]) < ord('0')) OR (ord($data[$i]) > ord('9')))	return false;
		}
		return true;
	}
	protected function lookAnTable($c) {
	 ...
	 protected function checkModeKanji($size, $data) {
		...
		for ($i=0; $i<$size; $i+=2) {
			...
			if (($val < 0x8140) OR (($val > 0x9ffc) AND ($val < 0xe040)) OR ($val > 0xebbf)) return false;
		}
		return true;
	}
	protected function check($mode, $size, $data) {
	 ...
	 protected function convertData($items) {
		...
		if ($ver > $this->version) $this->version = $ver;
		while (true) {
	 ...
	 protected function appendPaddingBit($bstream) {
	 	if (is_null($bstream)) return null;
		$bits = count($bstream);
		$maxwords = $this->getDataLength($this->version, $this->level);
		$maxbits = $maxwords * 8;
		if ($maxbits == $bits) return $bstream;
		if ($maxbits - $bits < 5) return $this->appendNum($bstream, $maxbits - $bits, 0);
		$bits += 4;
		$words = (int)(($bits + 7) / 8);
		$padding = array();
	...
	protected function getByteStream($items) {
	 ...
	 protected function appendBitstream($bitstream, $append) {
		if ((!is_array($append)) OR (count($append) == 0)) return $bitstream;
		if (count($bitstream) == 0) return $append;
	...
	protected function maximumWords($mode, $version) {
		...
		$words = (1 << $bits) - 1;
		if ($mode == QR_MODE_KJ) $words *= 2; // the number of bytes is required
		return $words;
	}
	...
	protected function getVersionPattern($version) {
		if (($version < 7) OR ($version > QRSPEC_VERSION_MAX)) return 0;
		return $this->versionPattern[($version - 7)];
	}
	protected function getFormatInfo($mask, $level) {
		if (($mask < 0) OR ($mask > 7)) return 0;
	...
	protected function newFrame($version) {
		if (($version < 1) OR ($version > QRSPEC_VERSION_MAX)) return NULL;
	...
	protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
		...
		if (($symsize < 0) OR ($symsize > 8)) return $rs;
		if (($fcr < 0) OR ($fcr >= (1<<$symsize))) return $rs;
		if (($prim <= 0) OR ($prim >= (1<<$symsize))) return $rs;
		if (($nroots < 0) OR ($nroots >= (1<<$symsize))) return $rs;
		if (($pad < 0) OR ($pad >= ((1<<$symsize) -1 - $nroots))) return $rs;
		$rs = array();
		$rs['mm'] = $symsize;
		$rs['nn'] = (1 << $symsize) - 1;
		$rs['pad'] = $pad;
