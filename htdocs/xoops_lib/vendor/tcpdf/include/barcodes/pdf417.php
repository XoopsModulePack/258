...
class PDF417 {
    ...
    public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
        ...
        if ($codewords[0] == 900) array_shift($codewords);
        // count number of codewords
        $numcw = count($codewords);
        if ($numcw > 925) return false;
        ...
        if (!empty($macro)) {
            ...
            if ($macro['segment_index'] == ($macro['segment_total'] - 1)) $macrocw[] = 922;
        ...
        if (!empty($macro)) $codewords = array_merge($codewords, $macrocw);
        ...
        foreach($numseq[1] as $seq) {
            ...
            if ($seq[1] > 0) {
                ...
                foreach($textseq[1] as $txtseq) {
                    ...
                    if ($txtseqlen > 0) $sequence_array[] = array(900, $txtseq[0]);
                    $txtoffset = $txtseq[1] + $txtseqlen;
                }
            }
            if ($seqlen > 0) $sequence_array[] = array(902, $seq[0]);
            $offset = $seq[1] + $seqlen;
        }
    ...
    protected function getCompaction($mode, $code, $addmode=true) {
        ...
        if ($addmode) array_unshift($cw, $mode);
        return $cw;
    }
}