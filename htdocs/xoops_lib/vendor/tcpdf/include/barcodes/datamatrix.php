...
if (!defined('DATAMATRIXDEFS')) define('DATAMATRIXDEFS', true);
define('ENC_ASCII', 0);
define('ENC_C40', 1);
define('ENC_TXT', 2);
define('ENC_X12', 3);
...
class Datamatrix {
	...
	public function __construct($code) {
		...
		foreach ($this->symbattr as $barcode_array) {
			if ($barcode_array[11] >= $nd) {
                $params = $barcode_array;
				break;
			}
		}
		if ($params[11] < $nd) {
	...
	protected function lookAheadTest($data, $pos, $mode) {
		...
		if ($pos >= $data_length) return $mode;
		...
		while (true) {
			...
			if (($pos + $charscount) == $data_length) {
				if ($numch[ENC_ASCII] <= ceil(min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF], $numch[ENC_BASE256]))) return ENC_ASCII;
				if ($numch[ENC_BASE256] < ceil(min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF]))) return ENC_BASE256;
				if ($numch[ENC_EDF] < ceil(min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_BASE256]))) return ENC_EDF;
				if ($numch[ENC_TXT] < ceil(min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_X12], $numch[ENC_EDF], $numch[ENC_BASE256]))) return ENC_TXT;
				if ($numch[ENC_X12] < ceil(min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_EDF], $numch[ENC_BASE256]))) return ENC_X12;
				return ENC_C40;
			}
			// get char
			...
			// STEP R
			if ($charscount >= 4) {
				if (($numch[ENC_ASCII] + 1) <= min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF], $numch[ENC_BASE256])) return ENC_ASCII;
				if ((($numch[ENC_BASE256] + 1) <= $numch[ENC_ASCII])
					OR (($numch[ENC_BASE256] + 1) < min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF]))) return ENC_BASE256;
				if (($numch[ENC_EDF] + 1) < min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_BASE256])) return ENC_EDF;
				if (($numch[ENC_TXT] + 1) < min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_X12], $numch[ENC_EDF], $numch[ENC_BASE256])) return ENC_TXT;
				if (($numch[ENC_X12] + 1) < min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_EDF], $numch[ENC_BASE256])) return ENC_X12;
				if (($numch[ENC_C40] + 1) < min($numch[ENC_ASCII], $numch[ENC_TXT], $numch[ENC_EDF], $numch[ENC_BASE256])) {
					if ($numch[ENC_C40] < $numch[ENC_X12]) return ENC_C40;
	...
	protected function getSwitchEncodingCodeword($mode) {
		...
		switch ($mode) {
			case ENC_ASCII: { // ASCII character 0 to 127
				$cw = 254;
				if ($this->last_enc == ENC_EDF) $cw = 124;
				break;
			}
			case ENC_C40: { // Upper-case alphanumeric
				$cw = 230;
	...
	protected function getMaxDataCodewords($numcw) {
		foreach ($this->symbattr as $matrix) {
			if ($matrix[11] >= $numcw) return $matrix[11];
		}
		return 0;
	}
