...
class TCPDF_FILTERS {
	...
	public static function decodeFilterASCIIHexDecode($data) {
		...
		if (preg_match('/[^a-fA-F\d]/', $data) > 0) self::Error('decodeFilterASCIIHexDecode: invalid code');
	...
	public static function decodeFilterASCII85Decode($data) {
		...
		if (strpos($data, '<~') !== false) $data = substr($data, 2);
		// check for EOD: 2-character sequence ~> (7Eh)(3Eh)
		$eod = strpos($data, '~>');
		if ($eod !== false) $data = substr($data, 0, $eod);
		// data length
		$data_length = strlen($data);
		// check for invalid characters
		if (preg_match('/[^\x21-\x75,\x74]/', $data) > 0) self::Error('decodeFilterASCII85Decode: invalid code');
		// z sequence
		...
		if ($group_pos > 1) $tuple += $pow85[($group_pos - 1)];
		// last tuple (if any)
		switch ($group_pos) {
			case 4: {
	...
	public static function decodeFilterFlateDecode($data) {
		...
		if ($decoded === false) self::Error('decodeFilterFlateDecode: invalid code');
		return $decoded;
	}
