...
class TCPDF_IMAGES {
    ...
    public static function getImageFileType($imgfile, $iminfo=array()) {
        ...
        if (isset($iminfo['mime']) AND !empty($iminfo['mime'])) {
            ...
            if ((count($mime) > 1) AND ($mime[0] == 'image') AND (!empty($mime[1]))) $type = strtolower(trim($mime[1]));
        }
        if (empty($type)) {
            $fileinfo = pathinfo($imgfile);
            if (isset($fileinfo['extension']) AND (!TCPDF_STATIC::empty_string($fileinfo['extension']))) $type = strtolower(trim($fileinfo['extension']));
        }
    ...
    public static function setGDImageTransparency($new_image, $image) {
        ...
        if ($tid >= 0) $tcol = imagecolorsforindex($image, $tid);
    ...
    public static function _parsejpeg($file) {
        ...
        if (count($icc) > 0) {
            ksort($icc);
            $icc = implode('', $icc);
            if ((ord($icc[36]) != 0x61) OR (ord($icc[37]) != 0x63) OR (ord($icc[38]) != 0x73) OR (ord($icc[39]) != 0x70)) $icc = false;
        } else {
            $icc = false;
        }
    ...
    public static function _parsepng($file) {
        ...
        //Check signature
        if (fread($f, 8) != chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10)) return false;
        //Read header chunk
        fread($f, 4);
        if (fread($f, 4) != 'IHDR') return false;
